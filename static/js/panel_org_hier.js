// ════════════════════════════════════════════════════════════════════════════
// ORG HIERARCHY – map/detail/tree/editor builders + layout engine + render
// ════════════════════════════════════════════════════════════════════════════

// ── SECTION: org-map (buildOrgMap) ────────────────────────────────────────────
  ...
  function walk(node){
    ...
    if(node.type==='person'){
      ...
      inner=`<circle cx="${px+22}" cy="${py+ph/2}" r="14" fill="${col}22" stroke="${col}44" stroke-width="1"/>
        ...
        <text x="${px+42}" y="${py+28}" fill="${C.faint}" font-size="9" font-family="Segoe UI,system-ui">${(node.role||'').slice(0,16)}</text>
      ...
      inner=`<rect x="${px}" y="${py}" width="${pw}" height="5" rx="3" fill="${col}"/>
        ...
        <text x="${px+12}" y="${py+37}" fill="${C.faint}" font-size="9" font-family="Segoe UI,system-ui">${sub}</text>`;
    }
...
// ── SECTION: org-detail (buildOrgDetail) ─────────────────────────────────────
    ...
    if(persons.length){
      body+=`<h3 style="margin-bottom:7px;">People</h3><div style="max-height:180px;overflow-y:auto;display:flex;flex-direction:column;gap:4px;">
        ${persons.map(p=>`<div class="flex-ac" style="padding:5px 8px;background:var(--bg3);border-radius:6px;cursor:pointer;" onclick="orgMapSelId='${p.id}';renderOrgHier()">
          ...
          <div class="label-xs">${p.role||''}</div></div></div>`).join('')}
      </div>`;
    }
  }
...
// ── SECTION: org-tree (buildOrgTreePanel) ────────────────────────────────────
...
// ── SECTION: org-editor (buildOrgEditor, orgSkillRow) ────────────────────────
...
// ── SECTION: layout engine + main render + map events ────────────────────────
